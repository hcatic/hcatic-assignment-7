...
def hypothesis_test():
    ...
    fun_message = "Rare event! p-value is extremely small!" if p_value <= 0.0001 else None
...
def confidence_interval():
    ...
    ci_lower, ci_upper = np.percentile(estimates, [(100 - confidence_level) / 2, 100 - (100 - confidence_level) / 2])
    ...
    plt.figure()
    plt.scatter(estimates, [observed_stat] * len(estimates), color="gray", alpha=0.5, label="Simulated Estimates")
    plt.scatter(mean_estimate, observed_stat, color="blue", s=100, label="Mean Estimate")
    plt.hlines(observed_stat, ci_lower, ci_upper, colors="blue", linewidth=2, label=f"{confidence_level}% Confidence Interval")
